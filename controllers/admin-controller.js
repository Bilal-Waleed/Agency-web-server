...
const getOrders = async (req, res) => {
  try {
    ...
    const orders = await Order.find()
      ...
      .select("orderId name email phone projectType projectBudget timeline projectDescription paymentReference paymentMethod files.name files.url createdAt avatar status")
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();
